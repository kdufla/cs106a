...
public class FacePamphletCanvas extends GCanvas implements
		FacePamphletConstants, ComponentListener {
	...
	private void addImage(GImage image) {
		...
		if (image == null) {
			GRect frame = new GRect(x, y, IMAGE_WIDTH*getWidth()/APPLICATION_WIDTH, IMAGE_HEIGHT*getHeight()/APPLICATION_HEIGHT);
			add(frame);
			GLabel noImage = new GLabel("No Image");
			noImage.setFont(PROFILE_IMAGE_FONT);
			add(noImage, x + IMAGE_WIDTH*getWidth()/APPLICATION_WIDTH / 2 - noImage.getWidth() / 2, y
					+ IMAGE_HEIGHT / 2 - noImage.getHeight() / 2);
		} else {
			image.setSize(IMAGE_WIDTH*getWidth()/APPLICATION_WIDTH, IMAGE_HEIGHT*getHeight()/APPLICATION_HEIGHT);
			add(image, x, y);
		}
	}
	...
	private void addStatus(String stat) {
		...
		add(status, LEFT_MARGIN, TOP_MARGIN + heightN + IMAGE_MARGIN
				+ IMAGE_HEIGHT*getHeight()/APPLICATION_HEIGHT + STATUS_MARGIN + status.getHeight());
	}
	...
	//add age
	private void addAge(String age) {
		GLabel friends = new GLabel("age: " + age);
		friends.setFont(PROFILE_FRIEND_LABEL_FONT);
